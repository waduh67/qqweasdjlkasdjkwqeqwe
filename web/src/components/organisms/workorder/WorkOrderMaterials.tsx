...
function Materials({ workOrder, context, settlement, reload }: { workOrder: WorkOrderView; context: MaterialFieldContext; settlement: MaterialSettlement; reload: () => void }) {
  ...
  return <>
    ...
    <div className="row wrap"><Button onClick={reload}>Muat ulang material WO</Button>{report && <Button variant="primary" onClick={() => setUsing(true)}>{context.plan?.materialMode === 'NONE' ? 'Nyatakan tanpa pemakaian material' : context.useRevision > 0 ? 'Catat tambahan pemakaian' : 'Catat pemakaian material'}</Button>}
