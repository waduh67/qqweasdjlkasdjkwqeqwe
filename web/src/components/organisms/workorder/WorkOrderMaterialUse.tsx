...
export function WorkOrderMaterialUse({ context, onDone, onClose }: { context: MaterialFieldContext; onDone: () => void; onClose: () => void }) {
  ...
  function submit(event: FormEvent) {
    ...
    try { const input = materialUseInput(context, rows, evidence, reason); setReview('lines' in input ? reportMaterialUse(context.workOrderId, input) : appendMaterialUse(context.workOrderId, input)); setError(null) }
    catch (caught) { setError(caught instanceof Error ? caught.message : 'Periksa pemakaian material.') }
  }
  return <><form className="card stack" aria-label="Catatan pemakaian material" onSubmit={submit}>
    <h3>{none ? 'Deklarasi tanpa material' : correction ? 'Tambahan pemakaian terukur' : 'Pemakaian material terukur'}</h3>
    ...
    <a href="#work-order-evidence">Lihat atau unggah bukti pengerjaan</a>
    ...
    {error && <p role="alert">{error}</p>}<div className="row wrap"><Button onClick={onClose}>Batal</Button><Button type="submit" variant="primary">Tinjau pemakaian</Button></div>
  </form>{review && <WarehouseCommandDialog title="Konfirmasi pemakaian material" command={review} confirmLabel="Catat pemakaian" onDone={onDone} onReload={onDone} onClose={() => setReview(null)} summary={<>
