    ...
    <section className="workspace-grid" aria-label="Daftar drift konfigurasi">
      {drift.map((item) => (
        <article className="card stack" key={item.id}>
          <div className="spread wrap"><div><Text as="h2" block size={400} weight="semibold">{DEVICE_LABEL[item.deviceKind]}</Text><Text as="span" block className="muted workspace-code" size={200}>{item.deviceId}</Text></div><StatusBadge status={item.status === 'NONE' ? 'ACTIVE' : item.status === 'BENIGN' ? 'WARNING' : 'CRITICAL'} label={driftLabel(item.status)} /></div>
          ...
          {(item.status === 'CONFLICTING' || item.status === 'UNKNOWN') && <div role="alert" className="workspace-callout warning">Jangan adopsi otomatis. Periksa observasi perangkat dan buat plan pengganti atau rollback eksternal.</div>}
        </article>
      ))}
    </section>
  )
      ...
      {props.capabilities.length === 0 ? <EmptyState title="Belum ada bukti kapabilitas" hint="Jalankan discovery adapter agar matriks dapat dievaluasi." /> : (
        <section className="workspace-grid" aria-label="Matriks kapabilitas adapter">
          {props.capabilities.map((capability) => {
            const certification = props.certifications.find((candidate) =>
            ...
            return (
              <article className="card stack" key={capability.id}>
                ...
                <div className="workspace-kv"><span>Perangkat</span><strong>{capability.deviceKind} / {capability.deviceId}</strong></div><div className="workspace-kv"><span>Firmware</span><strong>{capability.firmware}</strong></div><div className="workspace-kv"><span>Transport</span><strong>{capability.transport}</strong></div><div className="workspace-kv"><span>Operasi</span><strong>{capability.operationClass}</strong></div>{certification && <div className="workspace-kv"><span>Berlaku sampai</span><strong>{new Date(certification.validUntil).toLocaleString('id-ID')}</strong></div>}
