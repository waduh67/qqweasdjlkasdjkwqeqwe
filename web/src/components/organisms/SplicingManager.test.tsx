...
describe('SplicingManager', () => {
  ...
  it('membatasi grid responsif ke meja splicing tanpa mengubah grid core bersama', async () => {
    ...
    expect(css).toMatch(/\.splice-core-grid\s*\{[^}]*grid-template-columns:\s*repeat\(8,\s*minmax\(0,\s*1fr\)\);[^}]*width:\s*100%;/s)
    expect(css).toMatch(/\.splice-core-grid\s*>\s*\.core-chip\s*\{[^}]*width:\s*100%;[^}]*aspect-ratio:\s*1;/s)
    ...
    expect(css).toMatch(/\.core-grid\s*\{\s*display:\s*grid;\s*grid-template-columns:\s*repeat\(12,\s*minmax\(0,\s*1fr\)\);\s*gap:\s*0\.35rem;\s*width:\s*100%;\s*\}/s)
