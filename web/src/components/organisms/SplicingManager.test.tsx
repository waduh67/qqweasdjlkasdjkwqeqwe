...
describe('SplicingManager', () => {
  ...
  it('membatasi grid responsif ke meja splicing tanpa mengubah grid core bersama', async () => {
    ...
    expect(css).toMatch(/\.splice-core-grid\s*\{[^}]*grid-template-columns:\s*repeat\(8,\s*minmax\(0,\s*1fr\)\);[^}]*min-inline-size:\s*0;/s)
    expect(css).toMatch(/\.splice-core-grid\s*>\s*\.core-chip\s*\{[^}]*inline-size:\s*100%;[^}]*min-inline-size:\s*0;[^}]*min-block-size:\s*24px;[^}]*max-inline-size:\s*38px;[^}]*aspect-ratio:\s*1;/s)
    ...
    expect(css).toMatch(/@container\s*\(max-width:\s*18rem\)\s*\{\s*\.splice-core-grid\s*\{[^}]*grid-template-columns:\s*repeat\(4,\s*minmax\(0,\s*1fr\)\);/s)
    expect(css).toMatch(/\.core-grid\s*\{\s*display:\s*grid;\s*grid-template-columns:\s*repeat\(8,\s*minmax\(0,\s*36px\)\);\s*gap:\s*0\.25rem;\s*\}/s)
