...
export function MyMaterialReturn({ context, source, online, onDone, onClose }: {
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      myReturnInput(context, source, quantity, target?.id ?? null, reason, reference, serial)
      ...
      if (fresh.stockRevision !== source.stockRevision || fresh.quantityBase !== source.quantityBase || fresh.receiptId !== source.receiptId || fresh.sourceUsageId !== source.sourceUsageId || fresh.serial !== source.serial || current.workOrderRevision !== context.workOrderRevision)
        throw new Error('Sisa material atau WO berubah. Muat ulang sebelum mengembalikan.')
      setReview(returnMyMaterial(context.id, myReturnInput(current, fresh, quantity, location.id, reason, reference, serial)))
