...
it('uses named archived masters in historical stock filters without dropping the location restriction', async () => {
  ...
  await waitFor(() => expect(screen.getByRole('combobox', { name: 'Barang', exact: true })).toHaveProperty('disabled', false))
  fireEvent.change(screen.getByRole('combobox', { name: 'Barang', exact: true }), { target: { value: id.sku } })
  fireEvent.click(screen.getByRole('button', { name: 'Terapkan filter' }))
