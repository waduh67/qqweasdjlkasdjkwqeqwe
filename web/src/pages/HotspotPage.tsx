...
export function HotspotPage() {
  ...
  const exportVouchers = () => {
    if (visibleVouchers.length === 0) {
      toast.info('Tidak ada voucher sesuai filter untuk diekspor.')
      return
    }
    downloadVoucherExport(visibleVouchers, siteName, planName)
    toast.success(`${visibleVouchers.length} voucher berhasil diekspor.`)
  }
  ...
  const generate = async () => {
    ...
    if (!voucherDraft.siteId || !voucherDraft.planId || !Number.isInteger(quantity) || quantity < 1 || quantity > 1000 || !Number.isFinite(durationHours) || durationHours <= 0) {
      toast.error('Isi lokasi, paket, jumlah 1–1000, dan durasi yang valid.')
      return
    }
    setSaving(true)
    try {
      ...
      toast.success(`${created.credentials.length} voucher berhasil dibuat.`)
      await reload()
    } catch (err) {
  ...
  const saveSite = async () => {
    ...
    if (!siteDraft.nasId || !siteDraft.name.trim()) {
      toast.error('Pilih NAS dan isi nama lokasi.')
      return
    }
    setSaving(true)
    const payload = {
  ...
  const revoke = async (voucher: VoucherView) => {
    const reason = await prompt({ title: 'Cabut voucher', message: `Voucher ${voucher.username} akan langsung tidak dapat digunakan.`, label: 'Alasan pencabutan', confirmLabel: 'Lanjutkan', required: true, multiline: true })
    if (reason == null || !await confirm({ title: 'Konfirmasi pencabutan', message: `Cabut voucher ${voucher.username}? Tindakan ini tidak dapat dibatalkan.`, confirmLabel: 'Cabut voucher', danger: true })) return
    try {
  ...
  if (!canSiteView && !canVoucherView) return <div className="card"><Text as="h2" weight="semibold">Akses ditolak</Text><p className="muted">Kamu tidak punya izin melihat hotspot atau voucher.</p></div>

  return <div className="stack" style={{ gap: '1rem' }}>
    <PageHeader title="Hotspot & Voucher" subtitle="Kelola lokasi hotspot, portal akses, dan voucher sementara dari paket Hotspot." />
    ...
    {canSiteView && <div className="card pad-0"><div className="card-head"><Text as="h2" weight="semibold">Situs hotspot</Text></div><div className="card-body">{loading ? <Text className="muted">Memuat situs hotspot…</Text> : sites.length === 0 ? <EmptyState icon={<IconWifi size={34} />} title="Belum ada situs hotspot" hint="Tambahkan situs untuk menghubungkan NAS, mode portal, dan paket default." /> : <DataTable rows={sites} columns={siteColumns} rowKey={(site) => site.id} rowActions={siteActions} />}</div></div>}
