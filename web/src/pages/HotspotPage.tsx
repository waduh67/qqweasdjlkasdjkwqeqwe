...
import { Badge, Button, EmptyState, Segmented, SelectField, TextField, Toolbar } from '@/components/atoms'
...
export function HotspotPage() {
  ...
  const siteColumns: Column<HotspotSiteView>[] = [
    { key: 'name', header: 'Situs', sortValue: (site) => site.name, cell: (site) => `${site.name}${site.location ? ` · ${site.location}` : ''}` },
    { key: 'nas', header: 'NAS', sortValue: (site) => nasName(site.nasId), cell: (site) => nasName(site.nasId) },
    { key: 'mode', header: 'Mode portal', sortValue: (site) => site.portalMode, cell: (site) => PORTAL_MODE_LABEL[site.portalMode] },
    { key: 'plan', header: 'Paket default', sortValue: (site) => site.defaultPlanId ? planName(site.defaultPlanId) : '', cell: (site) => site.defaultPlanId ? planName(site.defaultPlanId) : 'Belum dipilih' },
    { key: 'branding', header: 'Branding', cell: (site) => site.branding.displayName ?? 'Default' },
  ]
  const voucherColumns: Column<VoucherView>[] = [
    { key: 'username', header: 'Kode voucher', sortValue: (voucher) => voucher.username, cell: (voucher) => voucher.username },
    ...
    { key: 'status', header: 'Status', sortValue: (voucher) => voucher.status, cell: (voucher) => STATUS_LABEL[voucher.status] },
  ...
  const batchColumns: Column<VoucherBatchView>[] = [
    ...
    { key: 'status', header: 'Status batch', sortValue: (batch) => batch.status, cell: (batch) => batch.status },
  ]
