import { integer, nullable, pageOf, record, text, uuid, WarehouseDataError } from './codec'
...
export const getMaterialApprovalReview = (id: string) => query(`${root(id)}/workbench/approval-review`, (value, path = 'approvalReview') => {
  ...
  return nullable(r.review, (value, path = 'review') => {
    const r = record(value, path), result = { id: uuid(r.id, path), workOrderRevision: integer(r.workOrderRevision, path), usage: materialUsage(r.usage, path) }
    if (result.usage.workOrderId !== id) throw new WarehouseDataError(path)
    return result
  }, path)
})
