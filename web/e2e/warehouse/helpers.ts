...
export async function signup(page: Page) {
  ...
  await page.getByLabel('Nama ISP', { exact: true }).fill(`Gudang ${randomUUID().slice(0, 8)}`)
  await page.getByLabel('Nama admin', { exact: true }).fill(admin.name)
  await page.getByLabel('Email admin', { exact: true }).fill(admin.email)
  await page.getByLabel('Password (min. 8 karakter)', { exact: true }).fill(admin.password)
...
export async function createUser(page: Page, role: string) {
  const user = identity('Pemeriksa')
  await page.goto('/users')
  await page.getByRole('button', { name: 'Pengguna baru', exact: true }).click()
  await page.getByLabel('Nama', { exact: true }).fill(user.name)
  await page.getByLabel('Email', { exact: true }).fill(user.email)
  await page.getByLabel('Password (min. 8 karakter)', { exact: true }).fill(user.password)
