...
SQL
    ...
    legacy_preflight_probe 178.9 MM ENFORCED > "$WAREHOUSE_LEGACY_RUN_DIR/preflight-$preflight_index-positive.txt"
    for gate in migration unit cutover; do
        case "$gate" in
            migration) version=170; unit=MM; cutover=ENFORCED; message='migration mismatch' ;;
            unit) version=178.9; unit=EA; cutover=ENFORCED; message='SKU unit mismatch' ;;
            cutover) version=178.9; unit=MM; cutover=LEGACY; message='cutover mismatch' ;;
        esac
...
for index in range(2):
    ...
    assert row['version'] == '178.9' and row['cutover'] == 'ENFORCED'
...
(root / 'preflight-verification.json').write_text(json.dumps({'status': 'PASSED', 'schemaBefore': '172',
    'schemaAfter': '178.9', 'readOnly': True, 'positiveProbes': 2, 'negativeProbes': 6,
