...
await_json() {
    ...
    for ((attempt=0; attempt<120; attempt++)); do
        if curl --noproxy '*' -fsS --max-time 2 -H 'Accept: application/json' "$url" >"$RUNTIME/warehouse-readiness.json" 2>/dev/null &&
            jq -e --arg marker "$WH_MARKER" --arg database "${WAREHOUSE_E2E_DATABASE:-warehouse_e2e}" 'type == "object" and .status == "UP" and .components.warehouse.details.database == $database and .components.warehouse.details.user == "warehouse_app" and .components.warehouse.details.marker == $marker' "$RUNTIME/warehouse-readiness.json" >/dev/null 2>&1; then
            return 0
        fi
        sleep 1
    done
