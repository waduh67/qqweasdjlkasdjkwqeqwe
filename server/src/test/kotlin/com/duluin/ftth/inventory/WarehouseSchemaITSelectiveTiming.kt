        ...
        database.dataSource.connection.use { connection -> connection.createStatement().use { statement ->
            ...
            """.trimIndent()).use { rows ->
                ...
                while (rows.next()) {
                    ...
                    val expected = WarehouseDeferredGuardCatalog.expected.getValue(rows.getString(1))
                    assertThat(WarehouseDeferredGuardCatalog.normalized(rows.getString(2).substringAfter("BEGIN")))
                        .describedAs(rows.getString(1)).startsWith(expected)
                }
                assertThat(names).containsExactlyInAnyOrderElementsOf(WarehouseDeferredGuardCatalog.expected.keys)
            }
        } }
    }
