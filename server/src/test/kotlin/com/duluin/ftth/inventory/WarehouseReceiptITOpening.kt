...
class WarehouseReceiptITOpening : WarehouseMasterHttpFixture() {
    @Test fun `opening balance requires independent approval and cannot impersonate supplier receipt`() {
        ...
        fun opening(actor: String, body: String = input, type: String = "application/pdf", bytes: ByteArray = ReceiptEvidenceFixtures.pdf()) = mvc.perform(multipart("/api/v1/warehouse/opening-balances/requests")
            .file(MockMultipartFile("file", "count.pdf", type, bytes))
        ...
        assertThat(opening(token, bytes = "%PDF-".toByteArray()).status).isEqualTo(400)
        assertThat(opening(token, bytes = "%PDF-1.4\n<html><script>alert(1)</script></html>".toByteArray()).status).isEqualTo(400)
