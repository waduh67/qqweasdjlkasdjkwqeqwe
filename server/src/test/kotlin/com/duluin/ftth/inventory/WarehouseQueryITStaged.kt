...
class WarehouseQueryITStaged : WarehouseReceiptHttpFixture() {
    @Test fun `unverified raw quantities and legacy identities are explicit without becoming available`() {
        ...
        val legacyDetail = mapper.readTree(request("GET","/api/v1/warehouse/assets/$asset",setup.token).contentAsString)
        assertThat(legacyDetail.path("quantity").path("baseUnit").isNull).isTrue()
        assertThat(legacyDetail.path("quantity").path("displayUnit").isNull).isTrue()
