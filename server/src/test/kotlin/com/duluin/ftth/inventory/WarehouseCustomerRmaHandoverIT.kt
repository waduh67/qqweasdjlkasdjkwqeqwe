...
class WarehouseCustomerRmaHandoverIT : WarehouseRepairFixture() {
    ...
    fun `inspected sold repair moves to its assigned technician for original customer without ISP availability`() {
        ...
        val namedHandover = request("GET", "/api/v1/warehouse/rma-handovers/$id/details", setup.token)
        assertThat(namedHandover.status).withFailMessage(namedHandover.contentAsString).isEqualTo(200)
        val named = mapper.readTree(namedHandover.contentAsString)
        assertThat(named.path("handover")).isEqualTo(handover)
        assertThat(named.path("workOrderCode").asString()).isNotBlank()
        assertThat(named.path("technicianName").asString()).isNotBlank()
        assertThat(named.path("senderName").asString()).isNotBlank()
        assertThat(named.path("locations").size()).isEqualTo(3)
        assertThat(named.has("email")).isFalse()
        ...
        val receivedDetails = request("GET", "/api/v1/warehouse/rma-handovers/$id/details", receipt.receiver.first)
        assertThat(receivedDetails.status).withFailMessage(receivedDetails.contentAsString).isEqualTo(200)
        assertThat(mapper.readTree(receivedDetails.contentAsString).path("handover")).isEqualTo(current)
