...
class WarehouseMigrationOpeningApprovalIT : WarehouseApprovalHttpFixture() {
    ...
    @Test fun `independent all-tier approval admits exact original physical identity and one competing baseline wins`() {
        ...
        try {
            ...
            val openingPages = (0..1).map { page ->
                val listed = request("GET", "/api/v1/warehouse/provenance/batches/${batch.id}/opening?size=1&page=$page", batch.token)
                assertThat(listed.status).withFailMessage(listed.contentAsString).isEqualTo(200)
                val contents = mapper.readTree(listed.contentAsString)
                assertThat(contents.path("totalElements").asInt()).isEqualTo(2)
                assertThat(contents.path("items").size()).isEqualTo(1)
                contents.path("items").path(0).path("id").asString()
            }
            assertThat(openingPages.toSet()).containsExactlyInAnyOrder(first, second)
            for (query in listOf("size=0", "size=101", "page=-1", "page=0&page=1", "extra=1"))
                assertThat(request("GET", "/api/v1/warehouse/provenance/batches/${batch.id}/opening?$query", batch.token).status).isEqualTo(400)
            val actors = tiers(batch.token, batch.old.location.toString(), 3)
            assertThat(request("GET", "/api/v1/warehouse/provenance/batches/${batch.id}/opening", actors[0].first).status).isEqualTo(403)
    ...
    @Test fun `empty legacy tenant posts explicit approved zero baseline without physical SKU line or valuation`() {
        ...
        val directory = "/api/v1/warehouse/provenance/batches/$batch/opening"
        val viewer = user(token, setOf("inventory.provenance.manage"))
        val hidden = request("GET", directory, viewer.first)
        ...
        assertThat(hidden.contentAsString).doesNotContain(document, "Confirmed no physical stock")
        grant(token, viewer.second, listOf(place.path("id").asString()))
        val visible = request("GET", directory, viewer.first)
        assertThat(visible.status).withFailMessage(visible.contentAsString).isEqualTo(200)
        assertThat(mapper.readTree(visible.contentAsString).path("items").path(0).path("id").asString()).isEqualTo(document)
    ...
    @Test fun `stopped application restarts with enforced A validating B original IDs and exact finalization replay`() {
        ...
        try {
            ...
            val openingHistory = getFresh(batch.token, "/api/v1/warehouse/provenance/batches/${batch.id}/opening")
            assertThat(openingHistory.statusCode()).withFailMessage(openingHistory.body()).isEqualTo(200)
            assertThat(mapper.readTree(openingHistory.body()).path("items").path(0).path("state").asString()).isEqualTo("POSTED")
