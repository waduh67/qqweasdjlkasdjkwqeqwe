            ...
            jdbc { connection ->
                ...
                val rejection = runCatching {
                    connection.createStatement().use { statement ->
                        statement.execute("""UPDATE inventory_balance_projection SET condition='DAMAGED',status='QUARANTINE',revision=revision+1
                            WHERE stock_identity_id='$asset' AND quantity_base>0""")
                        statement.execute("""UPDATE inventory_serialized_asset SET condition='DAMAGED',status='QUARANTINE',revision=revision+1
                            WHERE id='$asset'""")
