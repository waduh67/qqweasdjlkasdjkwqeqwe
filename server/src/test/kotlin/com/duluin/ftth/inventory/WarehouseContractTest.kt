...
class WarehouseContractTest {
    ...
    fun `state and ownership values are stable wire strings`() {
        val states = mapOf(
            ...
            "WarehouseReceiptState" to "DRAFT EXPIRED RECEIVED_IN_INSPECTION PUTAWAY CLOSED",
            "WarehouseTransferState" to "DRAFT EXPIRED DISPATCHED PART_RECEIVED RECEIVED DISCREPANCY",
            "MaterialDemandState" to "DRAFT SUBMITTED PART_RESERVED RESERVED PART_ISSUED ISSUED SETTLING CLOSED CANCELLED",
            "WarehouseIssueState" to "DRAFT PICKED UNPICKED DISPATCHED PART_RECEIVED RECEIVED",
            "WarehouseReturnState" to "DRAFT DISPATCHED RECEIVED_IN_INSPECTION ACCEPTED REPAIR SUPPLIER_RETURN SCRAP LOST",
            "WarehouseCountState" to "DRAFT EXPIRED COUNTING SUBMITTED APPROVED POSTED RECOUNT_REQUIRED",
    ...
    fun `stable errors distinguish malformed forbidden missing and conflict`() {
        ...
        val expected = mapOf(400 to "MALFORMED_REQUEST", 401 to "UNAUTHENTICATED", 403 to "FORBIDDEN",
            404 to "NOT_FOUND", 409 to "INSUFFICIENT_STOCK STALE_REVISION SOURCE_NOT_VERIFIED DRAFT_EXPIRED WRONG_CUSTODIAN ACTIVE_ASSIGNMENT_EXISTS APPROVAL_REQUIRED COUNT_STALE IDEMPOTENCY_CONFLICT STALE_AUTHORITY CUTOVER_REQUIRED STALE_CUTOVER USE_WORKORDER_ASSET_WORKFLOW INDEPENDENT_APPROVER_REQUIRED COST_BASIS_REQUIRED CURRENCY_MISMATCH MATERIAL_OBLIGATION_OUTSTANDING MATERIAL_UNPICK_REQUIRED")
