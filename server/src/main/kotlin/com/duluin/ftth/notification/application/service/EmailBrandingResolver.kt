...
data class ResolvedEmailIdentity(
    /** Alamat `From`; null = pakai `ftth.mail.from` dari env di adapter. */
    val fromAddress: String?,
    val fromName: String,
    /** Alamat `Reply-To`; hanya terisi bila tenant memakai alamatnya sendiri. */
    val replyTo: String?,
...
class EmailBrandingResolver(
    ...
    fun forTenant(tenantId: UUID): ResolvedEmailIdentity {
        ...
        val fromAddress = override?.fromAddress ?: platform.fromAddress
        return ResolvedEmailIdentity(
            fromAddress = fromAddress,
            fromName = override?.fromName
                ?: tenants.findById(tenantId)?.name
                ?: platform.fromName,
            // Hanya saat tenant benar-benar memakai alamatnya sendiri: mengarahkan balasan ke
            // alamat platform justru menyesatkan pelanggan yang membalas dengan pertanyaan.
            replyTo = override?.fromAddress,
    ...
    fun logoUrl(platform: PlatformEmailSettings, tenantId: UUID? = null, tenantHasLogo: Boolean = false): String? {
        val base = (platform.publicBaseUrl ?: mailProperties.publicBaseUrl.trim().takeIf { it.isNotEmpty() })
            ?.trimEnd('/')
            ?: return null
        return when {
