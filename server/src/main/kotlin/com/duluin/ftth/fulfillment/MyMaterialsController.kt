...
class MyMaterialsController(private val service: MyMaterialsService) {
    ...
    @GetMapping("/{id}/rmas") fun rmas(@PathVariable id: UUID, @RequestParam parameters: MultiValueMap<String, String>) = response(service.rmas(id, page(parameters)))
    @GetMapping("/{id}/rmas/{handover}") fun rma(@PathVariable id: UUID, @PathVariable handover: UUID, @RequestParam parameters: MultiValueMap<String, String>) = response(service.rma(id, handover).also { if (parameters.isNotEmpty()) invalid() })
...
class MyMaterialsService(private val authority: CurrentAuthorityApi, private val cutovers: InventoryTenantCutoverApi,
    ...
    fun rmas(id: UUID, page: WarehousePageRequest) = inventory.rmas(locked(id), page)
    fun rma(id: UUID, handover: UUID) = single(inventory.rmas(locked(id), WarehousePageRequest(0, 1), handover))
