...
class WarehouseApprovalStore(private val jdbc: WarehouseCommandJdbc) {
    ...
    fun event(operation: UUID): UUID = jdbc.execute { sql ->
        sql.query("SELECT id FROM inventory_outbox WHERE tenant_id=? AND operation_id=? AND event_kind IN ('RECEIVED','TITLE_REACQUIRED','RETURN_RECEIVED')", sql.tenant, operation) { it.uuid("id") }.single()
    }
