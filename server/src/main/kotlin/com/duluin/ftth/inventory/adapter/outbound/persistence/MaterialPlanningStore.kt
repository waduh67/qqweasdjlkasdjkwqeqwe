...
@Repository
class MaterialPlanningStore(private val jdbc: WarehouseCommandJdbc, private val lifetime: WarehouseDraftLifetimeStore) {
    ...
    fun current(workOrder: UUID): MaterialPlanHistory? = jdbc.execute { sql ->
        sql.query("""SELECT plan.id binding_plan_id,snapshot.snapshot,warehouse_plan_current_state(plan.tenant_id,plan.id,plan.state) state,submission.document_id FROM inventory_material_plan plan
            ...
            readHistory(snapshot, it.getString("state"), it.optionalUuid("document_id"))
        }.singleOrNull()
    }
    fun get(id: UUID): MaterialPlanHistory = jdbc.execute { sql ->
        sql.value("SELECT warehouse_assert_material_submission(?,?)", sql.tenant, id)
        sql.query("""SELECT snapshot.snapshot,warehouse_plan_current_state(plan.tenant_id,plan.id,plan.state) state,submission.document_id FROM inventory_material_plan plan
            ...
            readHistory(it.getString("snapshot"), it.getString("state"), it.optionalUuid("document_id"))
    ...
    fun history(workOrder: UUID, page: WarehousePageRequest): WarehousePage<MaterialPlanHistory> = jdbc.execute { sql ->
        ...
        val rows = sql.query("""SELECT plan.id binding_plan_id,snapshot.snapshot,warehouse_plan_current_state(plan.tenant_id,plan.id,plan.state) state,submission.document_id FROM inventory_material_plan plan
            LEFT JOIN inventory_material_plan_snapshot snapshot ON snapshot.tenant_id=plan.tenant_id AND snapshot.id=plan.id
            LEFT JOIN inventory_material_submission submission ON submission.tenant_id=plan.tenant_id AND submission.id=plan.id
    ...
    private fun readHistory(snapshot: String, state: String, document: UUID?): MaterialPlanHistory {
        val plan = mapper.readValue(snapshot, MaterialPlanSnapshot::class.java)
        val expiry = lifetime.plan(plan.id)
        return MaterialPlanHistory(plan, if (expiry == null) state else "EXPIRED", document, expiry)
    }
