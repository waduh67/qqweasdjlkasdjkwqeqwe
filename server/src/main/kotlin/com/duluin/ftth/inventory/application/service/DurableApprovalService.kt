...
class DurableApprovalService(private val cutovers: InventoryTenantCutoverApi, private val authority: CurrentAuthorityApi,
    ...
    private fun decideLocked(input: WarehouseApprovalDecisionInput, key: String, stopped: ApprovalPostingStopped?): WarehouseApprovalResponse {
        ...
        if (final) {
            val operation = PostingOperation(requireNotNull(operationId), "warehouse.approval.effect", record.id.toString(), current.fence.identity.userId,
                ...
                when (source.kind) { "TITLE_CORRECTION", "RETURN_TITLE" -> "TITLE_REACQUISITION"; "ADJUSTMENT" -> "TRANSFER_REMAINDER"; "COUNT" -> "COUNT_VARIANCE"; "LOSS", "SCRAP" -> source.kind; "DISPOSITION_REVERSAL" -> "DISPOSITION_REVERSED"; else -> "RECEIVE" }, 200, result, current.fence.epoch)
    ...
    fun rework(input: WarehouseApprovalReworkInput, key: String): WarehouseApprovalResponse {
        ...
        if (source.kind in setOf("LOSS", "SCRAP", "DISPOSITION_REVERSAL")) throw WarehouseContractException(WarehouseError(WarehouseErrorCode.SOURCE_NOT_VERIFIED,
