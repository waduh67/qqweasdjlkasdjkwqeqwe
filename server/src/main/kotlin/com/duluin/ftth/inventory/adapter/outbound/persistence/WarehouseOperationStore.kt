...
class WarehouseOperationStore(private val jdbc: WarehouseCommandJdbc) {
    fun lock(command: WarehousePreparedCommand): StoredWarehouseOperation? = jdbc.execute { sql ->
        sql.value("SELECT pg_advisory_xact_lock(hashtextextended(?,0))", "${sql.tenant}|${command.namespace}|${command.key}")
        sql.query("SELECT * FROM inventory_operation WHERE tenant_id=? AND namespace=? AND operation_key=?",
            sql.tenant, command.namespace, command.key) { row ->
