...
data class ReservationDemandLine(val id: UUID, val planLineId: UUID, val sku: UUID, val unit: StockUnit,
    val requested: Long, val continuous: Boolean, val tracking: String, val issued: Long = 0)
...
data class ReservationLineSupply(val demandLineId: UUID, val planLineId: UUID, val requestedBase: String,
    val reservedUnpickedBase: String, val reservedPickedBase: String, val backorderBase: String, val baseUnit: StockUnit, val issuedBase: String = "0")
...
internal object ReservationPlanning {
    ...
    fun wanted(line: ReservationDemandLine, selection: ReservationSelection?, existing: List<ReservationChange>): Long {
        ...
        val remaining = Math.subtractExact(Math.subtractExact(line.requested, reserved), line.issued)
    ...
    fun supplies(lines: List<ReservationDemandLine>, rows: List<ReservationChange>) = lines.map { line ->
        ...
        val backorder = Math.subtractExact(Math.subtractExact(line.requested, Math.addExact(unpicked, picked)), line.issued)
        if (backorder < 0) masterFailure(WarehouseErrorCode.INSUFFICIENT_STOCK)
        ReservationLineSupply(line.id, line.planLineId, line.requested.toString(), unpicked.toString(), picked.toString(), backorder.toString(), line.unit, line.issued.toString())
    }
    fun state(lines: List<ReservationLineSupply>): String = when {
        lines.all { it.issuedBase == it.requestedBase } -> "ISSUED"
        lines.any { it.issuedBase != "0" } -> "PART_ISSUED"
