...
class InventoryDeploymentService(private val cutovers: InventoryTenantCutoverApi, private val authority: CurrentAuthorityApi,
    ...
    override fun authorize(workOrderId: UUID, request: DeploymentIntentRequest, metadata: WarehouseMutationMetadata): DeploymentAuthorizationRef {
        ...
        when (request.purpose) {
            DeploymentPurpose.INSTALL -> if (request.previousAssignmentId != null) masterFailure(WarehouseErrorCode.MALFORMED_REQUEST)
            DeploymentPurpose.REPLACE -> if (request.previousAssignmentId == null) masterFailure(WarehouseErrorCode.MALFORMED_REQUEST)
            DeploymentPurpose.REMOVE, DeploymentPurpose.RETURN_CUSTOMER_RMA -> masterFailure(WarehouseErrorCode.MALFORMED_REQUEST)
        }
        ...
        val binding = prior?.permit?.binding ?: DeploymentBinding(UUID.randomUUID(), UUID.randomUUID(), current.fence.identity.tenantId,
            ...
            source.issueRevision, source.assetRevision, current.fence.epoch, cutover.snapshot.epoch, null,
            request.previousAssignmentId, request.previousAssignmentId?.let { store.previousRevision(it, source.customerId) })
        workOrders.lockAndValidate(DeploymentValidationContext(binding, current.fence, cutover))
        if (prior == null) store.lockPrevious(binding)
    ...
    private fun consumePurpose(request: ConsumeDeploymentRequest, metadata: WarehouseMutationMetadata, purpose: DeploymentPurpose,
        ...
        if (!preview.consumed) store.lockPrevious(preview.binding)
        ...
        val assignment = AssetAssignmentRef(binding.operationId, binding.assetId, binding.customerId, binding.workOrderId,
            ...
            AssetHandoverState.PENDING, null, started, null, binding.previousAssignmentId, binding.ownershipMode == AssetOwnershipMode.LOAN)
        ...
        assignments.append(NewAssetAssignment(assignment.assignmentId, binding.assetId, binding.customerId, binding.workOrderId,
            binding.issueLineId, binding.purpose, binding.ownershipMode, AssetLegalOwner.ISP, permit.source.provenance, binding.actorId, started, binding.previousAssignmentId))
    ...
    private fun validateSource(permit: DeploymentPermit, current: CurrentAuthority) {
        ...
        val expectedPlan = when (binding.purpose) {
            DeploymentPurpose.INSTALL -> "INSTALL" to "PSB"
            DeploymentPurpose.REPLACE -> "REPLACE" to "MIGRATION"
            DeploymentPurpose.REMOVE, DeploymentPurpose.RETURN_CUSTOMER_RMA -> masterFailure(WarehouseErrorCode.SOURCE_NOT_VERIFIED)
        }
        if (plan.state != "SUBMITTED" || plan.plan.id != source.planId || plan.plan.planRevision != binding.revisions.planRevision ||
            ...
            plan.plan.action != expectedPlan.first || plan.plan.workType != expectedPlan.second) masterFailure(WarehouseErrorCode.SOURCE_NOT_VERIFIED)
    }
