CREATE OR REPLACE FUNCTION monitoring_assert_discovery_receipt(scope uuid, target uuid, require_context boolean DEFAULT true)
...
DECLARE receipt monitoring_discovery_receipt; discovery discovered_onu;
    ...
    command jsonb; identity jsonb; canonical text; canonical_hash text; operation_hash text;
    installation jsonb; expected_topology jsonb; expected_response jsonb;
BEGIN
    ...
    SELECT source.canonical_payload,operation.payload_hash INTO canonical,operation_hash
    FROM inventory_command_identity source JOIN inventory_operation operation ON (operation.tenant_id,operation.id)=(source.tenant_id,source.id)
    WHERE source.tenant_id=scope AND source.id=result.operation_id;
    canonical_hash:=encode(sha256(convert_to(canonical,'UTF8')),'hex');
    IF canonical IS NULL OR canonical_hash IS DISTINCT FROM result.consume_hash OR canonical_hash IS DISTINCT FROM operation_hash THEN
        RAISE EXCEPTION 'discovery canonical bytes must match both committed consumption hashes' USING ERRCODE='23514';
    END IF;
    identity:=canonical::jsonb;
