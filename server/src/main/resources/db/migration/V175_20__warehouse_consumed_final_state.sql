...
BEGIN
    ...
    IF balance.status<>'CONSUMED' THEN
        IF balance.warehouse_admission='VERIFIED' AND balance.quantity_base>0 AND EXISTS (
            ...
            WHERE leg.tenant_id=NEW.tenant_id AND leg.stock_identity_id=balance.stock_identity_id AND leg.status='CONSUMED'
                AND leg.direction='IN' AND movement.state='APPLIED' AND
                (balance.base_unit='MM' OR (leg.location_id,leg.custody_owner_id,leg.custody_owner_kind,leg.condition,leg.legal_owner)=
                    (balance.location_id,balance.custody_owner_id,balance.custody_owner_kind,balance.condition,balance.legal_owner))) THEN
            RAISE EXCEPTION 'consumed material cannot become reusable custody' USING ERRCODE='23514';
        END IF;
        RETURN NEW;
    END IF;
    IF balance.warehouse_admission='VERIFIED' AND balance.quantity_base>0 THEN
