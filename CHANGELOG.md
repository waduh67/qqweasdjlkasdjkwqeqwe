...
**Diperbaiki**
- **Penutupan Dropdown / Combobox Saat Klik di Luar Panel (`Combobox`, `MultiCombobox`)**: Menambahkan listener `pointerdown`/`touchstart` pada capture phase di tingkat dokumen saat dropdown terbuka. Memastikan dropdown langsung tertutup ketika pengguna mengklik area kosong di luar blade/drawer (seperti kanvas peta MapLibre yang menangkap event mousedown).
