...
**Diubah**
- **Grid Core Kabel & Meja Splicing 8 Kolom Rapat & Kompak**: Menata ulang kisi core kabel (`.core-grid` pada `CableCoreManager`) dan meja sambungan (`.splice-core-grid` pada `SplicingManager`) menjadi 8 kotak berukuran presisi `32px` per baris dengan jarak rapat `0.3rem` (~5px) dan `width: fit-content`, menghilangkan ruang kosong renggang antar-kotak di dalam panel.
